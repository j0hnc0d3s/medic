...
// CSS  : src/pages/styles/Appointment.css  (shared)
// ─────────────────────────────────────────────────────────
...
const STAFF_NAV = [
  ...
  { img: clockImg, path: '/staff/appointments', title: 'Appointments', active: true },
...
export default function StaffAppointments() {
  ...
  const resetForm    = () => { setFormData(BLANK); setEditingAppt(null) }
  ...
  const filtered = appointments.filter(a => {
    ...
    let   matchDate   = true
    if (filterDate === 'today')    matchDate = a.appointmentDate?.toDate().toDateString() === new Date().toDateString()
    if (filterDate === 'upcoming') matchDate = a.appointmentDate?.toDate() >= new Date()
    if (filterDate === 'past')     matchDate = a.appointmentDate?.toDate() < new Date()
    return matchStatus && matchDate
  })

  const todayCount    = appointments.filter(a => a.appointmentDate?.toDate().toDateString() === new Date().toDateString()).length
  const upcomingCount = appointments.filter(a => a.appointmentDate?.toDate() >= new Date() && a.status !== 'cancelled').length

  if (loading) return <div className="appt-shell">
      {/* ── Left icon sidebar ──────────────────────── */}
...
<div className="appt-loading"><div className="appt-spinner"/></div></div>
      ...
      <div className="appt-page">
        ...
        <div className="appt-bar">
          <div className="appt-search">
            <img src={searchImg} alt="" className="appt-search-icon" />
            <span className="appt-search-text">
          ...
          <div className="appt-bar-right">
            ...
            <div className="appt-select-wrap">
              <select className="appt-select" value={filterStatus}
              ...
              return (
                <div key={appt.id} className="appt-card">
                  <div className="appt-card-left">
                    ...
                    {appt.appointmentTime && <div className="appt-card-time">{appt.appointmentTime}</div>}
                  </div>

                  <div className="appt-card-body">
                    <p className="appt-card-name">{appt.patientName}</p>
                    {appt.patientPhone && <p className="appt-card-phone">{appt.patientPhone}</p>}
        ...
        <div className="appt-modal-overlay" onClick={() => setShowModal(false)}>
          <div className="appt-modal" onClick={e => e.stopPropagation()}>
            <div className="appt-modal-head">
              <h2 className="appt-modal-title">{editingAppt ? 'Edit Appointment' : 'New Appointment'}</h2>
